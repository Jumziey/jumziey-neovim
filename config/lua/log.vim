function! Personallogclear() abort " {{{1
  call delete(s:logfile)
endfunction

" }}}1
function! Personalloglog(msg) abort " {{{1
...
" }}}1
function! Personallogautocmd(cmd) abort " {{{1
  let l:log = a:cmd
  ...
  call Personalloglog(l:log)
endfunction

" }}}1
function! Personallogautocmds() abort " {{{1
  ...
  if !s:activate
    call Personalloglog('Stopped autocmd log')
    return
  endif

  call Personalloglog('Started autocmd log')
  augroup log_autocmds
    for l:au in s:aulist
      silent execute printf(
            \ 'autocmd %s * call Personallogautocmd(''%s'')',
            \ l:au, l:au)
    endfor
  augroup END
endfunction
